...
3. Update the `sample-app.properties` file located in `<TOMCAT_HOME>/webapps/sample-app/WEB-INF/classes` with the relevant values.    
     ...
     <table>
      ...
      <tr>
        <td>SAML2.IdPURL</td>
        <td>Specifies the endpoint of Asgardeo to which login and logout request should be sent. This should be `https://api.asgardeo.io/t/<organization_name>/samlsso`. Change the organization name here.</td>
      </tr>
      <tr>
          <td>IdPPublicCert</td>
