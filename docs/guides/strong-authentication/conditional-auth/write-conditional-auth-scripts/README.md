...
To start off, let's [configure the conditional authentication](../configure-conditional-auth) to your application and check the default script once you enable the two steps authentication.
...
Now check whether user is a member of any of the group `admin` or `manager`. We have this utility function [isMemberOfAnyOfGroups(user, groups)](../conditional-auth-js-api-reference#ismemberofanyofgroups-user-groups) to check whether the user is a member of any of the groups in the list. 

Refer the [inbuilt functions](../conditional-auth-js-api-reference) to get to know more existing functions.
...
Similarly, you can build your own scripts to handle many scenarios using the [JS-API references](../conditional-auth-js-api-reference/) and [pre-defined templates](../conditional-auth-templates).
