...
The `redirect_uri` sent in <a :href="$withBase('/guides/applications/oidc/implement-auth-code/#get-authorization-code')">login</a> request and the `post_logout_redirect_uri` sent in <a :href="$withBase('/guides/applications/oidc/add-logout')">logout request</a> should match with one of the registered authorized redirect URLs.
